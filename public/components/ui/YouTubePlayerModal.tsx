...
export default function YouTubePlayerModal({ video, isOpen, onClose }: YouTubePlayerModalProps) {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4'>
      <div ref={modalRef} className='bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden'>
        {/* Header */}
          ...
          <div className='space-y-4'>
            ...
            {video.description && (
              <div>
                ...
                <p className='text-sm text-gray-600 line-clamp-3 leading-relaxed'>{video.description}</p>
              </div>
            )}

            <div className='flex items-center gap-6 text-sm text-gray-500'>
              {video.viewCount && <span>조회수: {parseInt(video.viewCount).toLocaleString()}회</span>}
